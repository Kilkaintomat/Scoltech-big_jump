...
out = audit / "reviews/active-followthrough-v2"
...
host_status = Path(os.environ["REVIEW_HOST_STATUS"])
state = host_status.read_text(encoding="utf-8")
...
finish(
    ...
    inputs=[source, p3 / "manifest.json", kimina / "manifest.json",
            Path(__file__), renderer, here / "protocol.json", here / "run.sbatch", host_status],
...
archive = audit / "review-packages/active-followthrough-v2.zip"
