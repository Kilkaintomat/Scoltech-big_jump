...
def test_kernel_self_reference_failure_stops_at_first_step(repl):
    trace = verify_trace(
        repl,
        "  exact noSelfTrace\n  trivial",
        trace_id="self-reference",
        problem_id="self-reference",
        header="open scoped Topology\ntheorem noSelfTrace : True := by",
    )
    assert not trace.whole_proof_ok and not trace.verified
    assert trace.t_star == 0, trace.model_dump()
    assert [s.status.value for s in trace.steps] == ["error", "unreached"]
    assert trace.check_absorbing()
