...
def test_unstaged_stages_resolve_the_same_explicit_runtime_and_never_fall_back(
    tmp_path, monkeypatch
):
    from onebigjump.lean.environment import discover

    original = tmp_path / "repl/.lake/build/bin/repl"
    original.parent.mkdir(parents=True)
    original.write_bytes(b"old runtime")
    reviewed = tmp_path / "reviewed-runtime"
    reviewed.write_bytes(b"reviewed runtime")
    monkeypatch.setenv("ONEBIGJUMP_REPL_BINARY", str(reviewed))
    assert discover(tmp_path).repl_binary == reviewed
    reviewed.unlink()
    assert discover(tmp_path).repl_binary is None
