...
def test_explicit_replay_budget_reaches_lean_tactic_context(repl):
    body = """  run_tac do
    let budget := Lean.maxHeartbeats.get (← Lean.getOptions)
    unless budget == 400000 do throwError "wrong heartbeat option {budget}"
    let activeBudget := (← readThe Lean.Core.Context).maxHeartbeats / 1000
    unless activeBudget == 400000 do throwError "wrong active heartbeat budget {activeBudget}"
  trivial"""
    trace = verify_trace(
        repl,
        body,
