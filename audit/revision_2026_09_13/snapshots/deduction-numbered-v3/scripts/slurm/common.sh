...
run_in_container() {
    local binds=(--bind /gpfs --bind /trinity --bind "$REPO" --bind "$HF_HOME")
    if [ -n "${E1_LOCAL_ELAN:-}" ]; then
        binds+=(--bind "$E1_LOCAL_ELAN:$ELAN_HOME")
    else
        binds+=(--bind "$ELAN_HOME")
    fi
...
# All stages refer to the same reviewed binary, including stages that do not execute Lean.
if [ -n "${E1_SNAPSHOT:-}" ] && [ -f "$E1_SNAPSHOT/configs/repl_runtime.json" ]; then
    obj_repl_paths="$(python3 -c 'import json,sys; p=json.load(open(sys.argv[1])); print(p["manifest"]); print(p["binary"])' "$E1_SNAPSHOT/configs/repl_runtime.json")"
    export E1_REPL_RUNTIME_MANIFEST="${obj_repl_paths%%$'\n'*}"
    export ONEBIGJUMP_REPL_BINARY="${obj_repl_paths#*$'\n'}"
fi
