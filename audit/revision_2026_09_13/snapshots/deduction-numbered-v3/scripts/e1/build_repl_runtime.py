...
folder = base / "runs/repl_runtime_20260912_v2"
...
frontend = project / "REPL/Frontend.lean"
f = frontend.read_text()
old_header = "let (env, messages) ← processHeader header opts messages inputCtx"
new_header = 'let (env, messages) ← processHeader header opts messages inputCtx\n    if messages.hasErrors then\n      let details ← messages.toList.mapM (fun msg => msg.data.toString)\n      throw <| IO.userError ("REPL header imports failed: " ++ "\\n".intercalate details)'
assert f.count(old_header) == 1
frontend.write_text(f.replace(old_header, new_header))
shutil.copy2(frontend, folder / "Frontend.lean")
config = {
    "base_repl_revision": "5d5c49d13dfc0c1d2df43a27c3e56e02ad81b9c3",
    "repair": "ProofSnapshot.create derives active Core.Context.maxHeartbeats from ContextInfo.options; header import errors are propagated before command processing discards them",
...
finish(
    folder,
    stage="lean-repl-runtime-build",
    context={"source": digest(source)},
    inputs=[
        source,
        Path(__file__),
        Path("/gpfs/gpfs0/denis.rakhmankin/onebigjump-tools/lean-runtime.tar.sha256"),
    ],
    outputs=[
        folder / "repl",
        folder / "Frontend.lean",
        folder / "Snapshots.lean",
        config_path,
        *sorted((folder / "source-original").rglob("*.lean")),
        *[
            folder / "source-original" / n
            for n in ("lakefile.toml", "lean-toolchain", "lake-manifest.json")
            if (folder / "source-original" / n).is_file()
        ],
    ],
    metrics=config,
)
