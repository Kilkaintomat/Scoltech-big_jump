...
def prompt(problem: dict[str, Any], version: str = "original-v1") -> str:
    rules = "\n".join(f"All {a} things are {b}." for a, b in problem["rules"])
    if version == "numbered-slots-v3":
        slots = "\n".join(f"{i}. Mira is <new property>." for i in range(1, problem["length"] + 1))
        return (
            "Complete a proof by forward inference using only the supplied rules.\n"
            "Each numbered line must derive one NEW fact by ONE rule from a fact "
            "already established. The given fact is available before line 1.\n"
            "Choose the path to the target; do not follow the irrelevant branch, "
            "repeat the given fact, repeat an earlier fact, or skip intermediate facts.\n\n"
            f"Given: Mira is {problem['initial']}.\nRules:\n{rules}\n"
            f"Target: Mira is {problem['goal']}.\n\n"
            f"Your answer has exactly {problem['length']} inference lines numbered "
            f"1 through {problem['length']}, followed by Answer: true.\n"
            "Use this layout, replacing each <new property> with a supplied property "
            "name such as p1234. Do not print angle brackets or extra lines.\n"
            f"{slots}\nAnswer: true\n\n"
            f"Line {problem['length']} must establish Mira is {problem['goal']}. "
            "Begin with line 1 and stop after Answer: true."
        )
    if version == "worked-example-v2":
        return (
            "Solve the final puzzle by forward inference. Output only its proof.\n"
            "Each proof line must be a NEW fact obtained from one rule. The GIVEN fact is "
            "available already and must NOT be printed or counted as an inference.\n"
            "Do not print rules, explanations, headings, numbering, or unrelated facts.\n\n"
            "SEPARATE EXAMPLE (not part of the final puzzle):\n"
            "Given: Mira is p0000.\n"
            "Rules: All p0000 things are p0001. All p0001 things are p0002. "
            "All p0002 things are p0003. All p0000 things are p9999.\n"
            "Target: Mira is p0003. Required new inferences: 3.\n"
            "Correct output for this example:\n"
            "Mira is p0001.\nMira is p0002.\nMira is p0003.\nAnswer: true\n"
            "The given p0000 is not repeated. The irrelevant p9999 is not needed.\n"
            "END EXAMPLE. Use only the following final puzzle's given fact and rules.\n\n"
            f"FINAL PUZZLE\nGiven: Mira is {problem['initial']}.\nRules:\n{rules}\n"
            f"Target: Mira is {problem['goal']}.\n"
            f"Output exactly {problem['length']} NEW inference lines, each in the form "
            "'Mira is p1234.'. The last inference must establish the target.\n"
            "Then output one separate line: Answer: true\n"
            "Begin directly with the first newly inferred fact."
        )
    if version != "original-v1":
        raise ValueError("unknown deduction prompt version: " + version)
    return (
...
def population(root: Path, source: Path) -> None:
    ...
    for length in range(3, 13):
        for role, count in (
            ("pilot_calibration", config.get("pilot_per_role_per_length", 2)),
            ("pilot_evaluation", config.get("pilot_per_role_per_length", 2)),
...
def generate(root: Path, phase: str, source: Path, shard: int, n_shards: int) -> None:
    ...
    if phase == "main":
        if not config.get("allow_main", True):
            raise ValueError("development-only protocol does not authorize main collection")
                        ...
                        [
                            {
                                "role": "user",
                                "content": prompt(
                                    r["problem"], config.get("prompt_version", "original-v1")
                                ),
                            }
                        ],
                        tokenize=True,
