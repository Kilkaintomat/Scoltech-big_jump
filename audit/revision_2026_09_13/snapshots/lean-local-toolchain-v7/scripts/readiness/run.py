...
from onebigjump.readiness import controls, deduction, diagnostics, p3_validation, p4, whitening
...
def main():
    ...
    with FileLock(str(args.folder / "stage.lock"), timeout=0):
        if (args.folder / "manifest.json").exists():
            ...
            return
        if args.stage == "p3-validation":
            p3_validation.run(args.folder, args.config, source, args.shard)
        elif args.stage == "p3-validation-summary":
            p3_validation.summarize(args.folder, args.root, args.config, source)
        ...
        elif args.stage == "control-diagnostics":
            diagnostics.run(args.folder, args.root, args.phase, source, args.shard)
