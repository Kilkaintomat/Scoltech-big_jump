...
def main():
    ...
    metrics=dict(started_utc=started,finished_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
        ...
        limitations=["Incomplete campaign; completed shards are not a random sample.",
    ...
    report=["# Проверка цифр и причин исходов Lean", "", "Срез: "+started, "",
    ...
    finish(OUT,stage="label-yield-read-only-audit",context=dict(source=digest(SOURCE),scope="diagnostic only"),
