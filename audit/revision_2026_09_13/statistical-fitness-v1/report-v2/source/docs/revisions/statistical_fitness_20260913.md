...
First finish verification of saved completions. Four independent Lean processes may run inside
one existing 32-CPU allocation, with unchanged proof budgets and one parent journal writer.
Validate exact trace accounting, semantic equality against serial verification, cleanup on
worker failure, and observed wall time/resource classifications before activation. Existing
running jobs retain their source and parameters. No regeneration is required.
