...
- Moment estimator is consistent for **every real `γ`**; GPD MLE for `γ > −1` (asymptotically normal
  for `γ > −1/2`). A calibrated upper confidence bound below 0 can support a bounded-tail interpretation. A CI containing 0 is inconclusive, not evidence of equivalence to H_alg; a scientific equivalence margin would need to be specified independently.
