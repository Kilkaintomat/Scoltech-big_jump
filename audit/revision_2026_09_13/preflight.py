...
metrics={'sessions':len(workers),'cases':len(all_results),'before_file_nr':before,'after_file_nr':files(),
 ...
 'passed':all(not r['unexplained_disagreement'] and r['category'] in ['verified','timeout_resource'] for r in all_results) and all(not w['remaining_processes'] for w in workers),
...
finish(out,stage='scoped-saved-proofs-two-session-preflight',context={'source':digest(source)},
 inputs=[source,runtime,Path(__file__),protocol,problems_path,manifest],outputs=[a,b],metrics=metrics)
