...
class LeanREPL:
    ...
    def __init__(
        ...
        *,
        ...
        self._import_reply: dict[str, Any] | None = None
    ...
    def start(self) -> None:
        """Retry only environment startup; every failure is retained before labels exist."""
        for attempt in range(self.startup_attempts):
            self._import_reply = None
            try:
                self._start_once()
                self.startup_diagnostics.append({"attempt": attempt + 1, "ok": True})
                return
            except (ReplError, TimeoutError) as exc:
                self.startup_diagnostics.append(
                    {
                        "attempt": attempt + 1,
                        "ok": False,
                        "error": str(exc),
                        "import_reply": self._import_reply,
                ...
                if attempt + 1 == self.startup_attempts:
                    raise
                log.warning("Lean startup attempt %d failed: %s", attempt + 1, exc)
    ...
    def _start_once(self) -> None:
        ...
        probe_source = "example : True := by trivial"
        if re.search(r"\bimport\s+Mathlib\b", self.imports):
            probe_source += "\nexample : Nat.Prime 2 := by norm_num"
        if re.search(r"\bimport\s+Aesop\b", self.imports):
            probe_source += "\nexample : True := by aesop"
        probe = self.command(probe_source, timeout_s=self.startup_timeout_s)
        if _errors(probe):
    ...
    def close(self) -> None:
        ...
        try:
            if proc.stdin:
                proc.stdin.close()
            # This process group was created by start_new_session. Its launcher may
            # exit before a child does, so waiting only for the launcher leaks REPLs.
            try:
                if os.name == "posix":
    ...
    def _exchange(self, payload: dict[str, Any], timeout_s: float | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if any(
                re.search(
                    r"remote i/o error|input/output error|stale file handle|too many open files",
                    message,
                    re.IGNORECASE,
                )
                for message in _errors(reply)
            ):
                raise ReplError("Lean infrastructure diagnostic: " + json.dumps(reply)[:2000])
            return reply
...
def _errors(reply: dict[str, Any]) -> list[str]:
    """Read elaboration, command and kernel-completion error channels.

    A kernel rejection can carry a proofState and empty goals while reporting its error
    only in proofStatus. It must stop replay immediately, just like an elaboration error.
    """
    out = [
        str(m.get("data", ""))
    ...
    status = reply.get("proofStatus")
    if isinstance(status, str) and status.strip().startswith("Error:"):
        out.append(status.strip())
    return out


def _is_resource_error(messages: list[str]) -> bool:
    """Lean resource exhaustion is neither a mathematical nor a segmentation failure."""
    return any(
        re.search(
            r"\(deterministic\) timeout|maximum (?:number of heartbeats(?: \(\d+\))?|recursion depth) "
            r"(?:has been )?(?:reached|exceeded)|(?:maxHeartbeats|maxRecDepth).*exceeded",
            message,
            re.IGNORECASE,
        )
    ...
    *,
    ...
    for i, seg in enumerate(segments):
        ...
        try:
            ...
            errs = _errors(reply)
            if errs:
                status = StepStatus.TIMEOUT if _is_resource_error(errs) else StepStatus.ERROR
