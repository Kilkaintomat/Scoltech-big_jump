...
OUT.mkdir()
capture=read_json(HERE/"timing-capture-final-v2.json");queue=capture["queue"]
sacct={}
...
inputs=[SOURCE,Path(__file__),HERE/"timing-capture-final-v2.json",HERE/"PROPOSAL.json",HERE/"tests-8466207.xml"]
...
formatted=[]
for line in lines:
 if formatted and formatted[-1].startswith("#") and line:formatted.append("")
 if line.startswith(("- ", "1. ")) and formatted and formatted[-1] and not formatted[-1].startswith("- "):formatted.append("")
 formatted.append(line)
(OUT/"REPORT.md").write_text("\n".join(formatted)+"\n",encoding="utf-8")
shutil.copyfile(HERE/"PROPOSAL.json",OUT/"PROPOSAL.json")
evidence=OUT/"evidence";evidence.mkdir()
for p in [HERE/"timing-capture-final-v2.json",HERE/"acceptance/metrics.json",HERE/"acceptance/replays.json",HERE/"tests-8466207.xml",io_xml,HERE/"io-benchmark/metrics.json"]:
...
for p in [HERE/"repair.py",Path(__file__),HERE/"verify-node.sbatch",HERE/"repair.sbatch",HERE/"io_benchmark.py",HERE/"io-benchmark/manifest.json"]:
