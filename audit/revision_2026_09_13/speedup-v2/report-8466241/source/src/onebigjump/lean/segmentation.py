...
def segment_proof(proof: str, *, base_line: int = 0) -> list[Segment]:
    ...
    for offset, raw in enumerate(lines):
        ...
        # Delimiters in strings, quoted identifiers, comments and character literals
        # cannot open a tactic continuation. Keep unmatched closing tokens for Lean
        # to diagnose; they must not make later independent tactics disappear.
        live = re.sub(r"'(?:\\.|[^'\\\n])'", "   ", live)
        for char in live:
            if char in closing:
                delimiters.append(closing[char])
            elif delimiters and char == delimiters[-1]:
                delimiters.pop()
        needs_operand = live.endswith(("<;>", ";")) or live.strip() == "all_goals"
