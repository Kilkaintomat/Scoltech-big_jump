...
HERE=BASE/"audit/revision_2026_09_13/activation-delivery-check-v3"
...
def checked_rows(path):
    result=[]
    expected_context=identity(read_json(path.with_suffix(".identity.json")))
    with path.open(encoding="utf-8") as f:
        for line in f:
            r=json.loads(line)
            expected=r.pop("row_sha256")
            assert identity(r)==expected,("row digest",path,r.get("trace_id"))
            assert r["context_sha256"]==expected_context
...
def main():
    ...
    for item in inventory:
        ...
        if model not in labels:
            ...
            inputs.append(lm)
        gen_path=ROOT/model/"main/generation"/p.parent.name/"samples.jsonl"
        gm=gen_path.parent/"manifest.json"
        gen_outputs={str(Path(k).resolve()):v for k,v in read_json(gm)["outputs"].items()}
        assert digest(gen_path)==gen_outputs[str(gen_path.resolve())]
        sample_rows=checked_rows(gen_path)
        samples={r["trace_id"]:{k:v for k,v in r.items() if k!="row_sha256"} for r in sample_rows}
        inputs.append(gm)
        rows=checked_rows(p.parent/"trajectories.jsonl")
        assert set(samples)=={r["trace_id"] for r in rows}
        ...
        for r in rows:
            ...
            for key in ["category","problem_id","role","temperature"]:
                assert r[key]==label[key],(key,r["trace_id"])
            sample=samples[r["trace_id"]]
            label_request={k:v for k,v in label.items() if k!="row_sha256"}
            assert identity(sample)==label["request_sha256"]
            assert identity([sample,label_request])==r["request_sha256"]
