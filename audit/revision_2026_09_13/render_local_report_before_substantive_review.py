...
para("Дополнительный дефект обнаружен полным прогоном: seed_everything передавал 64-битный seed в PYTHONHASHSEED, из-за чего дочерний Python отказывался запускаться. В v7 только PYTHONHASHSEED приведён к допустимому диапазону; seed Python random, NumPy и torch сохранены. Отдельные тесты реально запускают дочерние интерпретаторы. Полный повторный прогон требуется для допуска очереди.")
