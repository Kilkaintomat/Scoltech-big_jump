    ...
    with np.load(path,allow_pickle=False) as arr:
        for budget in selection_protocol["calibration_budgets"]:
            for j,method in enumerate(selection_protocol["methods"]):
                ...
                assert sorted(failures)==sorted(c["unavailable_reasons"]), (r["trace_id"],budget,method,failures,c["unavailable_reasons"])
                checks["reason_order_only_differences"]+=int(failures!=c["unavailable_reasons"])
...
p4_summary=completed_stage(base/"runs/expansion_20260911/p4/summary")["data"]
ded_v2_rows=rows(base/"runs/development_20260913/deduction/pilot/verification/labels.jsonl")
ded_v2_counts={"attempts":len(ded_v2_rows),"format_eligible":sum(r["format_eligible"] for r in ded_v2_rows),
               "categories":dict(Counter(r["category"] for r in ded_v2_rows))}
