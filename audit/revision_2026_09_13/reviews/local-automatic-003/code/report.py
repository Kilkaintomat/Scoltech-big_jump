...
for name in ("io_audit_v2.py","io-unit-8466025.xml","io-unit-8466025.log",
             "all-8466027.xml","local-validation-8466027.log",
             "8466027-preflight/manifest.json","8466027-preflight/metrics.json",
             "8466027-preflight/results-server-only.json"):
    attach(audit / name)
...
validation = {"job_id":"8466027","preflight":None,"test_counts":None}
if (audit/"8466027-preflight/metrics.json").exists():
    validation["preflight"] = read_json(audit/"8466027-preflight/metrics.json")
if (audit/"all-8466027.xml").exists():
    import xml.etree.ElementTree as ET
    suites=list(ET.parse(audit/"all-8466027.xml").getroot().iter("testsuite"))
...
para("Полная валидация v6 (Slurm 8466027): " + json.dumps(validation["test_counts"],ensure_ascii=False) + "; проверка сохранённых доказательств пройдена: " + str(bool(validation["preflight"] and validation["preflight"]["passed"])) + ".")
para("Промежуточный тестовый снимок v5 имел конфликт двух монтирований; исправление входит в v6. Очередь v6 допускается к запуску только после успешного задания 8466027. Подробная диагностика находится в metrics.json.")
