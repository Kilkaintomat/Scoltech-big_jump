...
if pilot_attestation:
    ...
    table(["Статус компилятора до содержательного ревью","Примеров"], [[k,v] for k,v in ar["statuses"].items()])
    para("Статусы source_exclusion_needs_text_review и resource_review_required относятся к первому автоматическому этапу. Все эти случаи затем разобраны ассистентом; итоговые пояснения по каждому примеру сохранены в case-review.json отдельного манифеста ревью.")
...
para("## Новый технический пилот: numbered-slots-v3")
...
para("## История исправлений и проверок")
para("Ниже сохранена история проверок v3/v4. Актуальная полная проверка v7 прошла и приведена выше; историческая составная валидация не подменяет её.")
...
para("## Предыдущий технический пилот: worked-example-v2")
