...
def attach(path, recursive=True):
    ...
    target = out / "artifacts" / path.relative_to(base)
...
queues = {}
for name in ("lean_reverification_20260913", "development_20260913", "controls_20260913"):
...
for model in ("deepseek", "goedel", "kimina"):
    root = base / "runs/lean_reverification_20260913" / model
...
for p in sorted(p3_root.glob("shard-*/manifest.json")): attach(p)
...
controls_root = base / "runs/controls_20260913"
controls = []
...
para("Сохранённые ответы моделей используются повторно без изменения токенов. Новая разметка хранится в runs/lean_reverification_20260913. Протоколы и основная ячейка анализа сохранены. Whitening/позиционные проверки ожидают её выходы. P4 повторно не обучался.")
tv = acceptance["tests"]
...
finish(out,stage="revision-status-report",context={"source":digest(source),"started_utc":started},
