...
## Продолжение 2026-09-13: исправленный Lean и новые проверки

Рабочий снимок: `audit/revision_2026_09_13/snapshots/lean-scopes-v4`.
Допуск и составная валидация: `audit/revision_2026_09_13/acceptance/manifest.json`.
Исправлены scoped namespaces, завершение дочерних процессов REPL и распознавание
ошибок ядра из proofStatus. Проверки сохранённых доказательств и повторных
двухсессионных запусков завершены. Подробности тестового прогона и исправления
гонки в самом тесте сохранены в метриках допуска.

Новая очередь `runs/lean_reverification_20260913` повторно размечает сохранённые
генерации без изменения основного протокола. Не использовать незавершённые шарды
как окончательный набор. Основные whitening/позиционные проверки в
`runs/controls_20260913` привязаны к её будущим измерениям.

Свежий технический пилот `runs/development_20260913/deduction` обработан полностью,
но не прошёл исходный порог формата; большая серия не разрешена. Проверяющий
алгоритм не менялся. Независимый аудит:
`audit/revision_2026_09_13/8465954-deduction-review/manifest.json`.

Независимая заранее заданная проверка кандидата P3 выполняется в
`runs/p3_validation_20260913`. До её завершения основной статистический метод
не меняется. P4 повторно не запускался; прежний отрицательный вывод о переходе
хвостового индекса сохраняется.
