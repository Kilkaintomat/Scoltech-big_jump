...
(out/"code").mkdir()
shutil.copy2(str(base/"audit/revision_2026_09_13/render_revision_report.py"),str(out/"code/report.py"))
...
if len(sys.argv)>2:
    ...
    command=["srun","--jobid="+allocation,"--overlap","--exact","--ntasks=1","--cpus-per-task=2",
             ...
             "bash",str(base/"audit/revision_2026_09_13/report.sbatch"),str(out)]
    subprocess.check_call(command)
    job=allocation
else:
    job=subprocess.check_output(["sbatch","--parsable","--job-name=obj0913-review-"+tag,"--output="+str(base/"audit/revision_2026_09_13"/("report-"+tag+"-%j.log")),str(base/"audit/revision_2026_09_13/report.sbatch"),str(out)],universal_newlines=True).strip()
