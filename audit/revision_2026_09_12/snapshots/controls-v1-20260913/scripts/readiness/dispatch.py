...
def tick(root, queue, host):
    ...
    for key, task in sorted(
        queue["tasks"].items(), key=lambda item: item[1].get("submission_priority", 0)
    ):
        ...
        command = [
            ...
            "--partition=" + partition,
            "--time={}:00:00".format(task.get("hours", 24)),
            "--output=" + str(root / "logs" / (task["job_name"] + "-%j.log")),
        ]
        command += (
            ["--gres=gpu:1", "--nice=" + str(task.get("nice", 10000))]
            if partition == "ais-gpu"
            else ["--exclude=cn69"]
        )
        task_source = task.get("source", queue["source"])
        command += [str(Path(task_source) / "scripts/readiness/run.sbatch")] + task["arguments"]
        result = subprocess.run(
            command,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            env=dict(os.environ, E1_SNAPSHOT=task_source),
        )
