    ...
    for i, seg in enumerate(segments):
        ...
        try:
            ...
            if max_heartbeats is not None:
                # The pinned REPL reconstructs proof snapshots with default options.
                # Set the trusted budget explicitly for every replayed segment.
                tactic = (
                    f"set_option maxHeartbeats {max_heartbeats} in\n"
                    + "first\n| "
                    + seg.text.replace("\n", "\n  ")
                )
            reply = repl.tactic(tactic, state, timeout_s=per_step_timeout_s)
            errs = _errors(reply)
            if errs:
