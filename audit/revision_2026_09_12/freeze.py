...
base=Path.cwd()
source=base/"audit/revision_2026_09_12/snapshots"/sys.argv[1]
source.mkdir(parents=True,exist_ok=False)
paths=[]
for folder in ("src","scripts","tests","configs","docs"):
    paths += [p for p in (base/folder).rglob("*") if p.is_file() and "__pycache__" not in p.parts and p.suffix not in (".pyc",".pyo")]
