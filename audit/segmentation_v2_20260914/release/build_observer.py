...
OUT=ROOT/'audit/segmentation_v2_20260914/release'
PROJECT=OUT/'repl'
...
replacement="""partial def findAllInfoTactics (t : InfoTree) (ctx? : Option ContextInfo) (rootGoals : List MVarId := []) :
  ...
  | .node info ts =>
    ...
    let tInfo := match info with
      | .ofTacticInfo i => if info.isOriginal && (i.isSubstantive || i.name? == some ``Lean.Parser.Tactic.«tactic_<;>_» || i.name? == some ``Lean.Parser.Tactic.paren) then [(info, ctx?, roots)] else []
      | _ => []
