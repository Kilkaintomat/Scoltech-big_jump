...
def attach_semantics(result,header,reply,trace_id=""):
    ...
    for pt in result['points']:
        ...
        if pt['execution_status']=='post_completion':pt['trace_label']='post_completion'
...
class SegmenterV2(base.Segmenter):
    def inspect(self,header,body,trace_id=""):
        ...
        if r.get('parse_ok'):attach_semantics(r,header,r['whole_reply'],trace_id)
        return r
    def replay_frontier(self,result,limit=100):
        ...
        if result.get('failure_and_prefix_reproduced'):
