...
O=Path(__file__).parent;ROOT=O.parents[1];dest=O/'validation';dest.mkdir(exist_ok=True)
...
# Retrieve only the 24 frozen sample answers; scanning journal text is retrieval, no inference.
samples={};retrieval=[];tokenizers={}
for model in ['deepseek','goedel','kimina']:
 target={x['trace_id'] for x in saved if x['model']==model}
 root=ROOT/'runs/lean_reverification_20260913_local'/model
 ...
 for path in sorted((root/'main/generation').glob('shard-*/samples.jsonl')):
  scanned=0
  with path.open(encoding='utf-8') as f:
   for line in f:
    scanned+=1;r=json.loads(line)
    if r['trace_id'] in target:
     samples[(model,r['trace_id'])]=r;target.remove(r['trace_id'])
    if not target:break
  retrieval.append({'model':model,'path':str(path),'rows_read':scanned})
  if not target:break
 if target:raise RuntimeError(('missing_saved_answers',model,sorted(target)))
write(O/'retrieval.json',retrieval)
write(O/'frozen-generations-small.json',[dict(model=m,**r) for (m,_),r in samples.items()])
