...
def main():
 ...
 with (control/"controller.lock").open("a") as lock:
  ...
  for name,expected_hash in read(Path("/beegfs/home/denis.rakhmankin/onebigjump/audit/followthrough_recovery_20260916_v1/source-manifest.json"))["files"].items():
   if hashlib.sha256(Path(name).read_bytes()).hexdigest()!=expected_hash:raise RuntimeError("Frozen flow source changed: "+name)
  plan=read(Path("/beegfs/home/denis.rakhmankin/onebigjump/audit/followthrough_recovery_20260916_v1/flow-plan-v3.json"));state_path=control/"jobs.json"
  ...
  def submit(name,spec,retry_indices=None):
   ...
   record={"id":job,"script":str(HERE/spec["script"]),"recovery_source":"/beegfs/home/denis.rakhmankin/onebigjump/audit/followthrough_recovery_20260916_v1/source-manifest.json","submitted_utc":datetime.datetime.now(datetime.timezone.utc).isoformat()}
