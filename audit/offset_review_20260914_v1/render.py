...
para('**ONE BIG JUMP — отчёт по результатам экспериментов**')
...
para('Все проценты следующей таблицы рассчитаны по всем допустимым ошибочным трассам. '
     'Из-за границ доказательства столбцы D=−1 и D=+1 не являются симметричным сравнением направлений; '
     'ниже отдельно приведены доступность соседей и общая подвыборка с обоими соседями.')
table(['Модель','D=−1','D=0','D=+1','Окно ±1','Случайное окно ±1','Всегда первый'],
 [[names[m],*[pct(v['subsets']['all']['rates'][k]) for k in ('lag_-1','lag_+0','lag_+1','near_1')],
  ...
  for m,v in M.items()])
para('**Границы доказательства и знаменатели при сравнении направлений.**')
para('На первом блоке ошибки D=−1 невозможен; на последнем блоке ошибки D=+1 невозможен. '
     'D вычисляется как разность двух существующих позиций, поэтому обращения к несуществующему шагу нет. '
     'В исходной таблице невозможное событие даёт ноль в числителе, но трасса остаётся в общем знаменателе. '
     'Это корректная частота по всей выборке, однако её нельзя напрямую интерпретировать как симметричное '
     'сравнение опережения и запаздывания.')
boundary={}
for m,v in M.items():
 joint=v['subsets']['all']['joint_length_failure_max']
 n=sum(r['count'] for r in joint)
 def count_if(predicate):return sum(r['count'] for r in joint if predicate(r))
 def rate(k,den):return k/den if den else None
 left=count_if(lambda r:r['failure_step']>1)
 right=count_if(lambda r:r['failure_step']<r['L'])
 left_hit=count_if(lambda r:r['max_step']-r['failure_step']==-1)
 right_hit=count_if(lambda r:r['max_step']-r['failure_step']==1)
 interior=[r for r in joint if 1<r['failure_step']<r['L']]
 nb=sum(r['count'] for r in interior)
 bl=sum(r['count'] for r in interior if r['max_step']-r['failure_step']==-1)
 br=sum(r['count'] for r in interior if r['max_step']-r['failure_step']==1)
 chance=sum(r['count']/r['L'] for r in interior)/nb if nb else None
 assert n==v['subsets']['all']['n']
 assert left_hit==v['subsets']['all']['offset_counts'].get('-1',0)
 assert right_hit==v['subsets']['all']['offset_counts'].get('1',0)
 boundary[m]={'all_traces':n,'left_eligible':left,'right_eligible':right,
  'left_hits':left_hit,'right_hits':right_hit,'left_conditional':rate(left_hit,left),
  'right_conditional':rate(right_hit,right),'both_eligible':nb,
  'both_left_hits':bl,'both_right_hits':br,'both_left_rate':rate(bl,nb),'both_right_rate':rate(br,nb),
  'both_uniform_each_direction':chance}
dsb=boundary['deepseek']
para(f"Например, у DeepSeek исходные {pct(dsb['left_hits']/dsb['all_traces'])} для D=−1 — это "
     f"{dsb['left_hits']}/{dsb['all_traces']}. Левый сосед существует только в {dsb['left_eligible']} трассах; "
     f"среди них частота равна {dsb['left_hits']}/{dsb['left_eligible']} = {pct(dsb['left_conditional'])}. "
     'Небольшая общая частота слева в значительной мере связана с недоступностью левого соседа.')
table(['Модель','Слева: попадания / доступно','Доля при доступном левом соседе',
       'Справа: попадания / доступно','Доля при доступном правом соседе'],
 [[names[m],f"{b['left_hits']} / {b['left_eligible']}",pct(b['left_conditional']),
   f"{b['right_hits']} / {b['right_eligible']}",pct(b['right_conditional'])] for m,b in boundary.items()])
para('У DeepSeek и Goedel в данной отобранной P2-подвыборке нет первых ошибок на последнем блоке, '
     'поэтому правый сосед существует во всех её трассах. Это свойство подвыборки с локализованной ошибкой, '
     'а не утверждение обо всех сгенерированных доказательствах.')
para('Условные доли слева и справа выше используют разные подвыборки. Для симметричного сравнения '
     'направлений нужны одни и те же доказательства с 1<t*<L, где оба соседа существуют. '
     'Первый скачок при этом не удаляется; положение исходного максимума сохраняется.')
table(['Модель','Трасс с обоими соседями','Максимум слева','Максимум справа',
       'Равномерный уровень каждого направления'],
 [[names[m],b['both_eligible'],f"{b['both_left_hits']} / {b['both_eligible']} — {pct(b['both_left_rate'])}",
   f"{b['both_right_hits']} / {b['both_eligible']} — {pct(b['both_right_rate'])}",
   pct(b['both_uniform_each_direction'])] for m,b in boundary.items()])
para('На общей подвыборке большого преимущества правого соседнего скачка уже не видно. '
     'Эти описательные частоты не являются самостоятельным тестом значимости: остаются различия семейств задач, '
     'типичные позиции максимумов, длины и зависимость попыток внутри одной задачи. '
     'Сильная асимметрия доступности соседей в исходной таблице не должна приниматься за направление механизма.')
para('Границы уже учтены в равномерном контроле: Pслуч(D=d при L,t*) = 1{1≤t*+d≤L}/L. '
     'Для окна ±1 используется число реально существующих позиций в окне, делённое на L. '
     'При трёх шагах и ошибке на первом окно содержит два шага, поэтому его случайное покрытие равно 2/3. '
     'Позиционные перестановки также выполняются между трассами одинаковой длины и не создают несуществующих соседей.')
(D/'boundary_metrics.json').write_text(json.dumps({
 'stage':'boundary-denominator-summary','source_metrics_sha256':sha(D/'metrics.json'),
 'job':os.environ['SLURM_JOB_ID'],
 'scope':'descriptive aggregates of saved joint length/failure/maximum counts; no new experiments or significance test',
 'models':boundary},ensure_ascii=False,indent=2)+'\n')
...
table(['Модель','Задач в контроле','D=+1: наблюдение','D=+1: позиционный контроль','p, одно направление','p, поиск по показателям'],
 [[names[m],v['subsets']['all']['position_null']['n_tasks'],
  ...
  for m,v in M.items()])
para('Частоты D=+1 в этом контроле практически совпадают с ожидаемыми при разрыве связи между конкретной ошибкой и скачком. '
     'Дополнительной индивидуальной связи по данному показателю не обнаружено. Это не доказательство отсутствия эффекта '
     'в других представлениях или более точной разметке; это ограничение интерпретации текущих высоких процентов.')
para('После первой ошибки последующие шаги помечены unreached: Lean их не исполнял. Текст полной генерации '
     'и внутренние состояния доступны, поэтому запаздывающий максимум можно измерить, но он не является реакцией '
     'модели на обратную связь от Lean: такой обратной связи при генерации не было.')
...
para('Нынешний формальный шаг может содержать целый вложенный блок тактик. Небольшое L не означает столько же '
     'элементарных логических действий. В частности, блок have ... := by может включать много строк, '
     'а ошибка внутри него локализуется пока до блока. Это существенная причина проверять более подробную сегментацию.')
...
outputs={str(D/n):sha(D/n) for n in ('REPORT_RU.md','REPORT_RU.html','offsets.png','offsets.pdf','onebigjumpdraft.pdf','boundary_metrics.json')}
if (D/'REPORT_RU.pdf').exists():outputs[str(D/'REPORT_RU.pdf')]=sha(D/'REPORT_RU.pdf')
receipt={'stage':'experiment-report-render','job':os.environ['SLURM_JOB_ID'],'created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
         'git_commit':os.environ.get('ONEBIGJUMP_GIT_COMMIT'),'git_dirty':bool(os.environ.get('ONEBIGJUMP_GIT_STATUS')),
         'environment':dict(manifest['environment'],hostname=__import__('platform').node(),platform=__import__('platform').platform(),python=sys.version,allocated_cpus=os.environ.get('SLURM_CPUS_PER_TASK')),'config':{'data_job':data['job'],'format':'Markdown + standalone HTML + plots','pdf_status':pdf_status},
