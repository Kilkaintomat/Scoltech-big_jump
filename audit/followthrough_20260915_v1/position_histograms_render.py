...
def axes_row(axes,m,metrics,ylimits=None):
    ...
    ticks=[0,1,zero-5,zero,zero+5,2*bound+1,2*bound+2]
    ticklabels=[f"≤−{bound+1}",f"−{bound}","−5","0","+5",f"+{bound}",f"≥{bound+1}"]
...
def page(metrics,case,fold,only=None):
    ...
    if nrows==3:
        fig.subplots_adjust(left=.065,right=.98,top=.81,bottom=.12,hspace=.73,wspace=.29)
    else:
        fig.subplots_adjust(left=.06,right=.98,top=.66,bottom=.22,wspace=.28)
