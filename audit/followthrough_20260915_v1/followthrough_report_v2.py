...
from followthrough_render_v2 import render
def main():
 start("followthrough_report_v2");folder=OUT/"followthrough_report_v2";folder.mkdir(exist_ok=True);assert not (folder/"manifest.json").exists()
 ...
 T(["Этап","Статус","Заявки"],[[name,(", ".join(sorted({x["state"]+(" ("+x["reason"]+")" if x.get("reason") else "") for x in s["members"]})) or s["state"]),", ".join(str(x.get("job","")) for x in s["members"])] for name,s in snapshot["stages"].items()],[122,90,279])
 ...
 finish("followthrough_report_v2",metrics,inputs,outputs)
 package=OUT/"followthrough_package_v2";package.mkdir(exist_ok=True);shutil.copytree(folder,package/"report",dirs_exist_ok=True)
 ...
 archive=OUT/"FOLLOWTHROUGH_20260915_v2.zip"
 with zipfile.ZipFile(archive,"w",zipfile.ZIP_DEFLATED) as z:
  for path in package.rglob("*"):
   if path.is_file():z.write(path,str(path.relative_to(package)))
 atomic(OUT/"followthrough-download-v2.json",{"archive":archive.name,"sha256":digest(archive),"files":len(files)+1,"pages":pages})
 print("FOLLOWTHROUGH REPORT READY",json.dumps(read(OUT/"followthrough-download-v2.json")),flush=True)
