...
def main():
 ...
 plan=start("new_tasks");check_manifest(OUT/"new_tasks/manifest.json")
 candidates=OUT/"new_tasks/candidates.json";allrows=read(candidates)
 rows=[r for r in allrows if int(identity(r["problem_id"])[:12],16)%args.shards==args.shard]
 dest=OUT/"new_tasks"/("reference-shard-"+str(args.shard));dest.mkdir(exist_ok=True)
 env=discover();repl=None;age=0;context={"plan":digest(HERE/"new_tasks-plan.json"),"source":digest(HERE/"new_tasks-source-manifest.json"),"candidates":digest(candidates),"shard":args.shard,"shards":args.shards}
 try:
  with Journal(dest/"results.jsonl",context) as journal:
   for problem in rows:
    tid=problem["problem_id"]
    if journal.existing(tid,identity(problem)) is not None:continue
    result={"trace_id":tid,"problem_id":tid,"exercise_group":problem["exercise_group"],"source_index":problem["source_index"]}
    ...
    try:
     ...
     match=re.search(r"(?m)^\s*(?:theorem|lemma)\s+"+re.escape(declaration)+r"(?=\s|[:(])",live)
     if match is None:raise SourceExclusion("reference_target_missing","reference does not declare exact target")
     parsed=formal_body("```lean4\n"+code[match.start():]+"\n```",problem["statement"],trusted_prefix(problem,plan["reference_heartbeats"]))
     if re.search(r"\b(sorry|admit|sorryAx)\b",mask_comments(parsed["body"],mask_strings=True)):
      raise SourceExclusion("reference_hole","reference has a literal proof hole")
     ...
     source=f'set_option maxHeartbeats {plan["reference_heartbeats"]}\n'+problem["directives"]+"\n"+problem["statement"]+parsed["body"]+"\n#print axioms "+declaration
  ...
  artifact_finish(dest,stage="corrected-proofnet-reference-verification",context=context,
   inputs=[candidates,HERE/"new_tasks-source-manifest.json",Path(env.repl_binary)],outputs=[mp,dest/"results.jsonl",dest/"results.identity.json",*[Path(r["detail_path"]) for r in results]],metrics=metrics)
 finally:
  if repl is not None:repl.close()
