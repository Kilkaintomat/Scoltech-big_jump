...
def main():
 ...
 with (control/"controller.lock").open("a") as lock:
  ...
  for name,expected_hash in read(HERE/"flow-v2-source-manifest.json")["files"].items():
   if hashlib.sha256(Path(name).read_bytes()).hexdigest()!=expected_hash:raise RuntimeError("Frozen flow source changed: "+name)
  plan=read(HERE/"flow-plan-v2.json");state_path=control/"jobs.json"
  ...
  def submit(name,spec,retry_indices=None):
   ...
   record={"id":job,"submitted_utc":datetime.datetime.now(datetime.timezone.utc).isoformat()}
