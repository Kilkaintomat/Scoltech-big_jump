...
def generate(model,role,plan):
  ...
  if pending:
   ...
   for lo in range(0,len(pending),16):
    ...
    for request,prompt,tokens,generated in zip(batch,prompts,ids,outs,strict=True):
     ...
     original_token_spans(tok,completion_ids,completion)
 ...
 metrics={"attempts":len(rows),"all_accounted":True,"exact_original_token_reconstruction":True,"logprob_alignment":True,
 ...
 artifact_finish(folder,stage="prospective-P2-generation",context=context,
  inputs=[HERE/"p2_pipeline-source-manifest.json",base/"manifest.json",base/(model+"-requests.json"),base/(model+"-config.json")],
