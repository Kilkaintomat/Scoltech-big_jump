...
recovery = base/'runs/lean_recovery_20260911'
now = datetime.datetime.now(datetime.timezone.utc)
out = audit/'reports'/now.strftime('%Y%m%dT%H%M%SZ')
out.mkdir(parents=True)
inputs = [source/'source-manifest.json', audit/'before-metrics.json', audit/'code-before.tar.gz']
...
accounting = subprocess.check_output(['sacct','-u','denis.rakhmankin','-S','2026-09-09','-X','--format=JobID,JobName%45,State,ExitCode,Elapsed,Start,End,NodeList','-P'],text=True)
...
with tarfile.open(audit/'code-before.tar.gz') as archive:
    old_files = {m.name:archive.extractfile(m).read() for m in archive.getmembers() if m.isfile() and '__pycache__' not in m.name and not m.name.endswith('.pyc') and m.name.startswith(('src/','scripts/','tests/','docs/e1/'))}
...
lines+=['','Прогоны частично перекрываются; количества не суммируются. Полный прогон относится к неизменяемому snapshot.','',
...
lines+=['Научные выводы P1–P3: **inconclusive**. Пилот содержит слишком мало независимых задач. Основная выборка, ручная проверка, статистическая калибровка и недостающие sensitivity/null остаются отдельными требованиями. Полный протокол статьи не завершён.','',
'## Воспроизводимость','', '- Серия: `'+str(recovery)+'`.','- Snapshot: `'+str(source)+'`.','- Очередь: `'+str(recovery/'queue.json')+'`.','- tmux: `onebigjump-recovery`.','- Изменения относительно начала этой ревизии: [revision.patch](revision.patch).','- Числа и детали: [metrics.json](metrics.json); [Slurm](slurm-accounting.tsv).','',
