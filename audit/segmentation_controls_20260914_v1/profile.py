...
OUT=REPO/"runs/segmentation_controls_20260914_v1"/("profile-"+os.environ.get("SLURM_JOB_ID","unallocated"))
def main():
    import torch
    ...
    OUT.mkdir(parents=True,exist_ok=False);check_source()
    ...
    for name in MODELS:
        ...
        if model.config.model_type not in ["qwen2","llama"]:raise ValueError("unsupported output-head architecture")
