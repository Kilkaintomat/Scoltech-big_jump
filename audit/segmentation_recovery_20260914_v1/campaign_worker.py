...
def main():
    ...
    try:
      with Journal(out/"observations.jsonl",ctx) as journal:
        if args.task not in config["recovery_tasks"]:raise ValueError("not an authorized recovery shard")
        inherit(journal,Path(config["parent_root"])/"shards"/f"{args.task:03d}",shard["sha256"])
