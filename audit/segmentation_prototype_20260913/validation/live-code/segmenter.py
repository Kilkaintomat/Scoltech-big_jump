...
SEARCH_PREFIX=("first","try","repeat","repeat'","any_goals","all_goals","solve","iterate")
def errors(reply):
    return [m for m in reply.get("messages",[]) if m.get("severity")=="error"]
...
def attach(result,full_source,header_char_shift,reply):
    ...
    for m in errors(reply):
        pos=position_char(full_source,m["pos"])-header_char_shift
...
class Segmenter:
    ...
    def replay_frontier(self,result,limit=60):
        ...
        for i in result["selected_node_ids"]:
            ...
            tested.append(dict(node_id=i,source=n["text"],ok=not errors(reply) and not reply.get("sorries"),reply=reply))
        result["context_replays"]=tested
        if fail is not None:
            target=next((r for r in tested if r["node_id"]==fail),None)
            result["failure_reproduced"]=target is not None and not target["ok"]
            result["prefix_reproduced"]=all(r["ok"] for r in tested if result["nodes"][r["node_id"]]["end_char"]<stop)
        return tested
