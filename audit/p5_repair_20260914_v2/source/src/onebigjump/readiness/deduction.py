...
def generate(root: Path, phase: str, source: Path, shard: int, n_shards: int) -> None:
    ...
    from .deduction_format import DECODING_BACKEND, guided_decoding_for_length
            ...
            for start in range(0, len(pending), 16):
                ...
                params = [
                    SamplingParams(
                        ...
                        logprobs=1,
                        guided_decoding=(
                            guided_decoding_for_length(r["problem"]["length"])
                            if config.get("syntax_guidance", False)
                            else None
                        ),
                    )
                    for r in batch
...
def analyze(root: Path, phase: str, source: Path) -> None:
    ...
    from ..experiments.p5_rate import fit_length_rate
    ...
    for temperature in config["temperatures"]:
        ...
        result: dict[str, Any] = {
            ...
            "rate_fit": fit_length_rate(counts),
