...
def main():
    ...
    runtime_receipt = read_json(HERE / "runtime_cc/mask-result.json")
    runtime_manifest = Path(runtime_receipt["manifest"])
    if digest(runtime_manifest) != runtime_receipt["sha256"]:
        raise ValueError("changed CUDA runtime result")
    verify_manifest(runtime_manifest)
    metrics["cuda_mask_runtime"] = read_json(runtime_manifest)["metrics"]
    found = {}
    inputs = [SOURCE, summary, preflight, Path(__file__), HERE / "review.sbatch", runtime_manifest, HERE / "runtime_cc/resume-job.json"]
    ...
    report.extend(["", "Для CUDA-маски подключён существующий компилятор кластера только в среде P5. Перед продолжением фактический CUDA-результат маски побитово сверен с CPU на полном словаре, подмножестве строк и дополненном словаре. Сохранённые обычные ответы повторно не генерировались.", "", "Calibration для активаций:", "",
