...
OUT=R/"runs/followthrough_recovery_20260916_v2/report_v3"
...
for stage in ["generation/shard-000-of-001","verification","extraction"]:
 ...
 elif (p.parent/"manifest.json").exists():p5metrics[stage]=read(p.parent/"manifest.json")["metrics"]
...
cats=p5metrics.get("verification",{}).get("categories",{})
assert cats.get("verified",0)+cats.get("invalid_inference",0)==p5["saved"]
summary=summary.replace("P5_RESULTS","Логически верны "+str(cats["verified"])+" ответов ("+format(100*cats["verified"]/p5["saved"],".2f")+"%); содержат ошибку вывода "+str(cats["invalid_inference"])+". Ошибок формата нет. Состояния извлечены для всех ответов. Это калибровочные результаты, а не итоговая независимая проверка зависимости от длины.")
...
summary=summary.replace("P5_GATE",("Допуск ещё не рассчитан." if p5gate is None else "Результат допуска: "+("пройден; отдельная оценочная серия поставлена в очередь." if p5gate["ready_for_evaluation"] else "не пройден.")))
...
print(json.dumps({"report":str(OUT),"as_of":now,"p2_saved":sum(x["saved"] for x in inv.values()),"p5_saved":p5["saved"],"p5_gate_passed":p5gate["ready_for_evaluation"]},ensure_ascii=False),flush=True)
