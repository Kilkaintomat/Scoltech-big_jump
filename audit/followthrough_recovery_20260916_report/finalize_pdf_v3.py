...
O=Path("/beegfs/home/denis.rakhmankin/onebigjump/runs/followthrough_recovery_20260916_v2/latex_final")
...
m={"pages":len(p),"missing_glyphs":False,"overfull_boxes":False,"scientific_metrics_unchanged":digest(O/"metrics.json")==digest(O.parent/"report_v3/metrics.json")}
(O/"pdf-checks.json").write_text(json.dumps(m,indent=2))
finish_artifact(O,stage="status-report-latex-pdf",context={},inputs=[O.parent/"report_v3/manifest.json",Path(__file__),Path(__file__).parent/"render_status_latex.py"],outputs=[O/n for n in ["REPORT.tex","REPORT.pdf","REPORT.md","REPORT.html","metrics.json","REPORT-latex.zip","pdf-checks.json"]],metrics=m)
print(json.dumps(m))
