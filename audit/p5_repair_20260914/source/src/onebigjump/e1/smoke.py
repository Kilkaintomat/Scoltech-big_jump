...
from .verification import theorem_name, trusted_prefix, verify_one
...
def fixtures() -> list[tuple[str, str, str, str]]:
    return [
        ...
        ("all_goals_operand", "True ∧ True", "  constructor\n  all_goals\n  trivial", "verified"),
        ("outdented_combinator", "True ∧ True", "  constructor\n<;> trivial", "verified"),
...
def run(root: Path, source_manifest: Path) -> None:
    ...
    with LeanREPL(
        ...
        for name, goal, body, expected in fixtures():
            ...
            if name == "correct":
                problem["statement"] = problem["statement"].replace(pid, "e1_fixture_Correct")
                completion = completion.replace(pid, "e1_fixture_Correct")
            if name == "unicode_eos":
                completion = (
        ...
        for problem in read_json(root / "inputs/problems.json"):
            ...
            absent = repl.command(f"#check {theorem_name(problem)}", timeout_s=180)
            reply = repl.command(
    ...
    finish(
        ...
        + (
            [Path(os.environ["E1_REPL_RUNTIME_MANIFEST"])]
            if os.environ.get("E1_REPL_RUNTIME_MANIFEST")
            else []
        ),
