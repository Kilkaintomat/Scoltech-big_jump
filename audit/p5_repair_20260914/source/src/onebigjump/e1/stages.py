...
def generation_inputs(root: Path, phase: str) -> tuple[list[dict[str, Any]], list[Path]]:
    ...
    for manifest in manifests:
        verify_manifest(manifest, checked)
...
def verify(root: Path, phase: str, source: Path, shard: int = 0, n_shards: int = 1) -> None:
    ...
    workers = int(os.environ.get("E1_VERIFY_WORKERS", "1"))
    if not 1 <= workers <= 4:
        raise ValueError("E1_VERIFY_WORKERS must be between 1 and 4")
    if workers > 1:
        from .parallel_verification import verify_stream

        if int(os.environ.get("SLURM_CPUS_PER_TASK", "0")) < 8 * workers:
            raise ValueError("parallel verification requires eight allocated CPUs per worker")
        context["execution"] = {"workers": workers, "mode": "independent-lean-processes"}
        with Journal(directory / "labels.jsonl", context) as journal:
            pending = [s for s in samples if journal.existing(s["trace_id"], identity(s)) is None]
            requests = {s["trace_id"]: identity(s) for s in pending}
            for result in verify_stream(pending, problems, settings, workers):
                journal.append(result, requests[result["trace_id"]])
                print(
                    "verified",
                    len(journal.rows),
                    "/",
        ...
        startup_diagnostics = [v["execution"] for v in values]
    else:
        with Journal(directory / "labels.jsonl", context) as journal:
            with LeanREPL(
            ...
            values = list(journal.rows.values())
        startup_diagnostics = repl.startup_diagnostics
    metrics = {
        "attempts": len(values),
        ...
        "explained_resource_disagreements": sum(
            v.get("explained_resource_disagreement", False) for v in values
        ),
        "resource_limited": sum(v.get("resource_limited", False) for v in values),
...
def lean_fingerprint(settings: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime_path = os.environ.get("E1_REPL_RUNTIME_MANIFEST")
    runtime_digest = None
    if runtime_path:
        runtime = Path(runtime_path)
        verify_manifest(runtime)
        runtime_digest = digest(runtime)
        if digest(env.repl_binary) != read_json(runtime)["outputs"][str(runtime.parent / "repl")]:
            raise ValueError("active REPL binary differs from the reviewed runtime manifest")
    return {
        "toolchain": env.toolchain,
        "mathlib_revision": env.mathlib_rev,
        "repl_revision": expected_repl,
        "repl_local_patch_manifest_sha256": runtime_digest,
