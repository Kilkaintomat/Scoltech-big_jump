...
def grammar_checks(deduction, grammar_for_length, tokenizer, vocab_size):
    ...
    for length in range(3, 13):
        ...
        long = gold.replace("\nAnswer: true", "\nMira is p1234.\nAnswer: true")
        params = GuidedDecodingParams(grammar=grammar, backend="xgrammar:no-fallback")
        SamplingParams(max_tokens=1024, guided_decoding=params)
...
def main():
    ...
    source_manifest = write_once(SOURCE_ROOT / "source-manifest.json", {
        ...
        "metrics": {**oldmeta, "dirty": True, "amendment": "isolated-p5-rate-and-syntax-v1",
    ...
    operational = [HERE / name for name in (
        ...
        "gpu_entry.py", "pilot.sbatch", "preflight.py", "preflight.sbatch", "study-protocol.json",
    ...
    report.extend(["", "## Отдельный парный запуск", "",
