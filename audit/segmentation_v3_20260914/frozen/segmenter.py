...
Intentional conservative frontier:
  ...
  descend cases/induction and calc steps; keep search and mapping combinators atomic.
...
SAFE={"Lean.Parser.Tactic.tacticHave__","Lean.Parser.Tactic.tacticLet__",
      ...
      "Lean.Parser.Tactic.focus","Lean.Parser.Tactic.cases","Lean.Parser.Tactic.induction",
      "Lean.calcTactic","Lean.calcFirstStep","Lean.calcStep"}
def errors(reply):
...
def from_ast(parse_text:str,ast:dict)->dict:
    ...
    def seq_scan(n,parent):
        ...
        elif n["kind"] in {"Lean.calcFirstStep","Lean.calcStep"}:
            add(n,parent)
        else:
...
class Segmenter:
    ...
    def replay_frontier(self,result,limit=60):
        ...
        for i in result["selected_node_ids"]:
            ...
            s=states[0]
            reply=self.repl._exchange(dict(tactic=n["text"],proofState=s["proof_state"]),timeout_s=30)
            tested.append(dict(node_id=i,source=n["text"],ok=replay_status(reply)=="ok",status=replay_status(reply),reply=reply,replay_source="exact_AST_slice",stored_text_matches=s["tactic"].strip()==n["text"].strip()))
