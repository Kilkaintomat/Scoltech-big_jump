...
from segmenter_v2 import align_original,reuse_plan
from segmenter_fast import SegmenterFast as SegmenterV2
...
fixed=read(O/'corrected-fixture.json')['case']
for c in cases:
 if c['name']=='replay_source_with_goal_and_comment':
  c.update(header=fixed['header'],body=fixed['body'])
...
write(dest/'manifest.json',{'job':os.environ['SLURM_JOB_ID'],'files':{str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [O/'segmenter.py',O/'segmenter_v2.py',O/'validate.py',O/'sample.json',O/'runtime-manifest.json',*dest.glob('*.json')] if p.name!='manifest.json'}})
