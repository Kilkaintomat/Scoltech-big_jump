...
structure ObservationRange where
  first : Nat
  last : Nat
  kind : String
  descend : Bool := false
...
structure CommandOptions where
  observeRanges : Option (Array ObservationRange) := none
...
structure CommandResponse where
  ...
  observerProfile : Option Json := none
deriving FromJson
...
instance : ToJson CommandResponse where
  toJson r := Json.mkObj <| .flatten [
    ...
    match r.observerProfile with | some j => [("observerProfile", j)] | none => []
  ]
