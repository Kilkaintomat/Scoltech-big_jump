...
accounting = subprocess.check_output(['sacct','-X','-n','-j',','.join(sorted({t['job_id'] for q in (queue,original) for t in q['tasks'].values() if t.get('job_id')})), '--format=JobID,JobName,State,ExitCode,Elapsed','-P']).decode()
