...
def main():
 ...
 artifact_finish(folder,stage="prospective-P2-formal-entry-states",context=context,
  inputs=[RECOVERY/"source-manifest.json",HERE/"p2_pipeline-source-manifest.json",*source],outputs=[mp,folder/"trajectories.jsonl",folder/"trajectories.identity.json",*[Path(r["states_path"]) for r in rows if r.get("states_path")]],metrics=metrics)
