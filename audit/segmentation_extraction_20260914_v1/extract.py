...
def main():
    ...
    p=argparse.ArgumentParser();p.add_argument("--model",choices=MODELS,required=True);p.add_argument("--wave",choices=["ready","remaining"],required=True)
    args=p.parse_args();source=check_source();config=read(RUN/"config.json");plan=read(RUN/"plan.json")
    tasks=[t for t in plan[args.wave] if plan["models"][str(t)]==args.model]
    if args.wave=="remaining":
    ...
    if model.config.model_type not in ["qwen2","llama"]:raise ValueError("unvalidated output head architecture")
    ...
    for task in tasks:
        ...
        context={"source":digest(RUN/"source-manifest.json"),"config":digest(RUN/"config.json"),"upstream":digest(upstream),"task":task}
        with Journal(out/"trajectories.jsonl",context) as journal:
            for item,annotation,blob_path in shard_items(task):
                ...
                row={"trace_id":rid,"model":args.model,"problem_id":item["problem_id"],"sample_meta":{k:item["sample"][k] for k in ["temperature","role","model_id","model_revision","tokenizer_revision"]},
                     ...
                     "input_sha256":identity(item),"execution":{"job":os.environ["SLURM_JOB_ID"],"host":os.uname().nodename}}
                if annotation.get("ready_for_activation_extraction") and annotation["status"]=="annotated":
                    ...
                    original=np.asarray(s["generation_token_logprobs"],dtype=float)
                    if original.shape!=completion_lp.shape:raise ValueError("generation logprob shape")
                    row.update(extraction_status="extracted",states_path=str(artifact),states_sha256=digest(artifact),
                               ...
                               forward_check=checks,backend_logprob_mean_abs_error=float(np.mean(np.abs(original-completion_lp))))
        ...
        metrics={"attempts":len(values),"all_accounted":True,"statuses":dict(collections.Counter(r["extraction_status"] for r in values)),
                 "model":args.model,"task":task,"generation_calls":0}
        finish(out,metrics,[upstream,RUN/"source-manifest.json"],[out/"trajectories.jsonl",out/"trajectories.identity.json",
